...
func TestMissingKnownDependency(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name: "detect cuda",
			err:  fakeErr("dlopen /tmp/remembrances-libs-123/libggml.so: libggml-cuda.so: cannot open shared object file: No such file or directory"),
			want: "libggml-cuda.so",
		},
		{
			name: "detect metal",
			err:  fakeErr("dlopen /tmp/remembrances-libs-123/libggml.so: libggml-metal.so: image not found"),
