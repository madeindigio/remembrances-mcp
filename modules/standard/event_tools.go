...
func init() {
	modules.RegisterModule(EventToolsModule{})
}

// EventToolsModule provides event tools.
type EventToolsModule struct {
	toolManager *mcp_tools.ToolManager
	tools       []modules.ToolDefinition
}

// ModuleInfo returns module metadata.
func (EventToolsModule) ModuleInfo() modules.ModuleInfo {
	return modules.ModuleInfo{
		ID:          "tools.events",
		Name:        "Event Tools",
		Description: "Registers MCP tools for events",
		Version:     "1.0.0",
		Author:      "Remembrances Team",
		License:     "MIT",
		New:         func() modules.Module { return new(EventToolsModule) },
	}
}

// Provision configures the module.
func (m *EventToolsModule) Provision(ctx context.Context, cfg modules.ModuleConfig) error {
	if cfg.Storage == nil {
	...
	if err := m.toolManager.RegisterEventToolsWith(reg); err != nil {
		return err
	}
...
func (m *EventToolsModule) Tools() []modules.ToolDefinition {
	return m.tools
}
