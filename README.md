...
```bash
# Quick build (current platform, 5-8 minutes)
make build

# Multi-platform build (all platforms, 6-10 minutes)
...
- `--llama-model-path`: Path to .gguf model file for llama.cpp embeddings
- `--llama-dimension`: Dimension of embeddings generated by llama.cpp model (default: 768)
- `--llama-threads`: Number of threads to use for llama.cpp (0 = auto-detect)
- `--llama-gpu-layers`: Number of GPU layers to offload to GPU (0 = CPU only)
- `--llama-context`: Context size for llama.cpp model (default: 512)
