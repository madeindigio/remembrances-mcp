...
func TestCreateEmptyResultTOON(t *testing.T) {
	msg := "no results"
	suggestions := AlternativeSuggestions{OtherIDs: []string{"u1 (3)", "u2 (1)"}}
	out := CreateEmptyResultTOON(msg, suggestions)
	if !containsAll(out, []string{"message: no results", "u1 (3)", "u2 (1)"}) {
		t.Fatalf("TOON does not contain expected content: %s", out)
	}
}
